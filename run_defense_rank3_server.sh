...
echo "Starting rank=3, alpha=0.5 on GPU 0"
CUDA_VISIBLE_DEVICES=0 nohup python -u -m src.scripts.run_defense_comparison \
  ...
  --rank 3 \
  --alpha 0.5 \
  > logs/defense_r3_server.log 2>&1 &

PID=$!
echo "$PID" > logs/defense_r3_server.pid
echo "Started PID=$PID"
echo "Monitor: tail -f logs/defense_r3_server.log"
