...
if ! command -v "${PYTHON_BIN}" >/dev/null 2>&1; then
  ...
  echo "  PYTHON_BIN=/full/path/to/python ./run_proposed_server1.sh"
  exit 1
fi
...
run_one () {
  ...
  CUDA_VISIBLE_DEVICES=1 "${PYTHON_BIN}" -u     -m src.scripts.run_defense_comparison     --method proposed     --basis "${BASIS}"     --shadow-updates "${SHADOW}"     --rank "${rank}"     --alpha "${alpha}"     2>&1 | tee -a "${log_file}"

  echo "Completed rank=${rank}, alpha=${alpha} at $(date)"
}


run_one 3 0.25 "logs/proposed_r3_a025.log"
run_one 3 0.75 "logs/proposed_r3_a075.log"

echo "SERVER 1 EXPERIMENTS COMPLETED"
